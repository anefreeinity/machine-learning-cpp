...
namespace ANEFreeInIty
{
    ...
    private:
        ...
        std::vector<std::vector<std::vector<double>>> _hiddenLayerWeights;
        std::vector<std::vector<double>> _outputLayerWeights;

        std::vector<std::vector<double>> _hiddenLayerBiases;
        std::vector<double> _outputLayerBiases;

        std::vector<std::vector<double>> _hiddenLayerCalculatedOutput;
    ...
    public:
        DeepNeuralNetwork();
        DeepNeuralNetwork(int, std::vector<int>, int, double learningRate = 0.1, std::string relativePath = "./", bool extractWeightsAndBiases = false);
